...
/**
 * In-memory SSE push service for IM real-time messaging.
 * Manages a registry of active SSE emitters per IM session.
 */
@Slf4j
@Service
public class ImPushServiceImpl implements ImPushService {
    ...
    private static class EmitterEntry {
        ...
        EmitterEntry(ResponseBodyEmitter emitter, Long sessionId, Long userId, String token) {
    ...
    // Per-IM-session subscriber emitters
    ...
    @Override
    public ResponseBodyEmitter subscribe(Long sessionId, Long userId, String token) {
        // Create emitter with timeout
        ResponseBodyEmitter emitter = new ResponseBodyEmitter(SSE_TIMEOUT_MS);

        EmitterEntry entry = new EmitterEntry(emitter, sessionId, userId, token);
        ...
        log.info("[IM-PUSH] User {} subscribed to IM session {}, token={}", userId, sessionId, emitterToken);

        // Auto-cleanup when emitter completes/error/timeout
        emitter.onCompletion(() -> {
            removeEntry(emitterToken);
            log.info("[IM-PUSH] Emitter completed, token={}", emitterToken);
        });
        emitter.onError(e -> {
            removeEntry(emitterToken);
            log.warn("[IM-PUSH] Emitter error, token={}", emitterToken, e);
        });
        emitter.onTimeout(() -> {
            removeEntry(emitterToken);
            log.info("[IM-PUSH] Emitter timeout, token={}", emitterToken);
        });
    ...
    public void pushToSession(Long sessionId, String eventType, Object data) {
        ...
        if (entries == null || entries.isEmpty()) {
            log.debug("[IM-PUSH] No subscribers for IM session {}", sessionId);
            return;
        }
        ...
        for (EmitterEntry e : entries) {
            try {
                e.emitter.send(sseData.getBytes("UTF-8"));
            } catch (IOException ex) {
                dead.add(e);
                log.warn("[IM-PUSH] Failed to push to emitter, removing", ex);
            }
        }
